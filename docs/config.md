...
| `playback_refresh_duration_in_ms` | Interval (ms) between playback refreshes; `0` disables polling, positive values have a 1000 ms minimum. | `1000`                                                                    |
...
- `app_refresh_duration_in_ms` controls rendering, not API polling. Playback polling defaults to `playback_refresh_duration_in_ms=1000`; positive values below 1000 use a 1000 ms minimum. Set it to `0` for event/command-only refreshes. Existing configurations explicitly set to `0` keep that behaviour. Local events update play/pause and progress immediately when their track and device match; remote updates depend on API latency and rate limits. Concurrent refreshes are coalesced.
